...
class CKA:
    ...
    def __init__(
        ...
        model1: nn.Module,
        model2: nn.Module,
        layers: Sequence[str | int] | None = None,
    ...
    def _accumulate_hsic(
        ...
        if self._is_same_model:
            ...
            if self.model1_layer_to_module.keys() == self.model2_layer_to_module.keys():
                hsic_matrix = hsic_cross(grams1, grams2)
    ...
    def _compare(
        ...
        with torch.inference_mode():
            for batch_idx, batch in enumerate(iterator):
                ...
                if x.shape[0] <= 3:
                    raise ValueError(
                        f"HSIC requires batch size > 3, got {x.shape[0]}. "
...
def compute_cka(
    ...
    *dataloaders: DataLoader,
    ...
    if len(dataloaders) == 1 and isinstance(dataloaders[0], (list, tuple)):
        dataloaders = tuple(dataloaders[0])

    if not dataloaders:
        raise ValueError("compute_cka requires at least one dataloader.")
